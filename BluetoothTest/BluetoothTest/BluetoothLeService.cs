...
namespace BluetoothTest
{
	...
	[Service (Name = "bluetoothtest.BluetoothLeService", Enabled=true)]			
	public class BluetoothLeService : Service
	{
		...
		public readonly static UUID UUID_HEALTH_THERMO_MEASUREMENT =
			UUID.FromString (SampleGattAttributes.HEALTH_THERMO_MEASUREMENT);
		IBinder mBinder;
		...
		public void BroadcastUpdate (String action, BluetoothGattCharacteristic characteristic) 
		{
			...
			if (UUID_HEALTH_THERMO_MEASUREMENT.Equals(characteristic.Uuid)) 
			{
				GattProperty flag = characteristic.Properties;

				var bytedata = characteristic.GetValue ();
				var healthTemp = characteristic.GetFloatValue (GattFormat.Float, 1);
				var timestamp = characteristic.GetIntValue (GattFormat.Uint8, 3);
				var temptype = characteristic.GetIntValue (GattFormat.Uint8, 4);

				Log.Debug (TAG, String.Format ("Received health temperature: {0}", healthTemp));
				intent.PutExtra (EXTRA_DATA, healthTemp.ToString());
			}
			else 
			{
     	...
     	* It is called only once when application is starting.
    	...
    	* Disconnects an existing connection or cancel a pending connection. The disconnection result
		...
		public void SetCharacteristicNotification (BluetoothGattCharacteristic characteristic, bool enabled)
		{
			...
			//This is specific to Health Thermometer
			/*if (UUID_HEALTH_THERMO_MEASUREMENT.Equals(characteristic.Uuid)) 
			{
